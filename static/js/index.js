...
function drawRoad() {
  ...
  for (let x = 0; x < canvas.width; x += stripeWidth + stripeSpacing) {
    ctx.fillRect(
      x,
      canvas.height - roadHeight / 2 - stripeHeight / 2,
      stripeWidth,
      stripeHeight
    );
  }
...
function updateCoinImages() {
  coins.forEach((coin) => {
    coin.img =
      coin.type === "good"
        ? imageElements.goodCoinImg
        : imageElements.badCoinImg;
  });
}
...
async function gameLoop(timestamp) {
  ...
  coins.forEach((coin) => {
    ...
    if (
      ...
      console.log(
        `Collision detected between player and ${coin.type} coin at (${coin.x}, ${coin.y})`
      );
      ...
      if (coin.type === "good") {
        ...
        if (score < 0) score = 0;
        breakdownMessage = `<span style="${
          shitCoinPoints < 0 ? "color:red;" : "color:green;"
        }">${shitCoinPoints} Triend points collected</span>`;
      }
  ...
  obstacles.forEach((obstacle) => {
    ...
    if (
      ...
      console.log(
        `Collision detected between player and ${obstacle.type} obstacle at (${obstacle.x}, ${obstacle.y})`
      );
    ...
    if (
      ...
      showMessage(
        '<span style="color:red;">Market Maker Bear Has Stolen Your Gains</span>'
      );
      obstacles = obstacles.filter((o) => o !== obstacle);
      //actions
      score = score - 50;
    ...
    if (
      ...
      if (now - lastHurtTime > 3000) {
        if (obstacle.type === "triend") {
          ...
          updateScore(
            `<span style="color:green;">200 Triend points collected</span>`
          );
          ...
          updateScore(
            `<span style="color:green;">Infinite knowledge grants you +100 Triend points</span>`
          );
  ...
  if (Math.random() < coinSpawnRate * deltaTime * 60) {
    const coinType = doubleGoodCoinSpawn
      ? "good"
      : Math.random() < 0.1
      ? "bad"
      : "good";
  ...
  const greenCandleProbability = baseGoldTokenProbability * (1 / 60); // 0.083%
  const redCandleProbability = baseGoldTokenProbability * 0.02; // 0.1%
  ...
  if (Math.random() < bearProbability * deltaTime * 60) {
    obstacles.push({
      ...
      y: Math.random() * (canvas.height - 200),
      width: player.width,
      height: player.height * (0.7 + Math.random() * 0.3),
      speed: 5,
      type: "bear",
    });
  }
  ...
  if (Math.random() < greenCandleProbability * deltaTime * 60) {
    let random_value = player.height * (0.7 + Math.random() * 0.3);
    obstacles.push({
      x: canvas.width,
...
window.onclick = function (event) {
  if (
    !event.target.matches("#add-points-button") &&
